...
def text_clean(text):
    ...
    dat = dat.apply(lambda x: re.sub(r'\<[^<>]*\>', ' ', x))
    ...
    dat = dat.apply(lambda x: [i for i in x if len(i)>=2])
    dat = dat.apply(lambda x: ' '.join(x))
...
class Seq2seq(object):
    ...
    def fit_model (self,
        encoder_input_data,
        decoder_input_data,decoder_target_data,tokenizer):
...
if __name__ == "__main__":
    ...
    epochs = 28 #28  # Number of epochs to train for. # maybe 14
    ...
    dom = xml.dom.minidom.parse('corpus.xml')
    root = dom.documentElement
    itemlist = root.getElementsByTagName('text')
    dialogue_lines = []
    for item in itemlist:
        dialogue_lines.append(item.firstChild.data)
    dialogue_lines = text_clean(pd.Series(dialogue_lines))
    ...
    dialogue_lines = dialogue_lines[:(len(dialogue_lines)-1)]
    ...
    # 2.9428
    s2smodel.fit_model(encoder_input_data,decoder_input_data,decoder_target_data,tokenizer)
    ...
    # Manual Testing:
    query0 = 'good one'
    query = tokenizer.texts_to_sequences([query0])
    query2 = np.zeros(
        (1, max_len, max_words),
        dtype='float32')
